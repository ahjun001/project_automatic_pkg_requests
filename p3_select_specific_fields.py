...
p3_label_header_s = ''  # header to be printed in front of label group in output
...
def save_label_info_json():
    ...
    global p3_header_height
    global p3_page_view_box_w
    global p3_page_view_box_h

    p3_fields_info_d['selected_fields'] = p3_selected_fields_l
    p3_fields_info_d['label_header'] = p3_label_header_s
    p3_fields_info_d['header_height'] = p3_header_height
    p3_fields_info_d['page_view_box_w'] = p3_page_view_box_w
    p3_fields_info_d['page_view_box_h'] = p3_page_view_box_h
...
def render_all_labels_with_default_specific_fields():
    ...
    if drs:
        ...
        for p3_fields_rel_dir in drs:
            ...
            if load_p3_fields_info_f():
                p3_selected_fields_l = p3_fields_info_d['selected_fields']
                p3_label_header_s = p3_fields_info_d['label_header']
                p3_header_height = p3_fields_info_d['header_height']
                p3_page_view_box_w = p3_fields_info_d['page_view_box_w']
                p3_page_view_box_h = p3_fields_info_d['page_view_box_h']
            else:
                # use the Chinese name in the directory located after the last '_'
                p3_label_header_s = p3_fields_rel_dir[p3_fields_rel_dir.rfind('_') + 1:]
            # complement with default_fields anyway
...
def render_all_label_all_products():
    ...
    if drs:
        ...
        for p3_fields_rel_dir in drs:
            ...
            # update disk version in label subdir
            save_label_info_json()
            # opening a new page
            p3_fields_abs_dir = os.path.join(p1.p1_contract_abs_dir, p3_fields_rel_dir)
            # printing labels in pages svg
            svg_in = os.path.join(p3_fields_abs_dir, 'label_template_header.svg')
            with open(svg_in) as h:
                header = h.read()
            if page == 1:
...
{label_nr}. {p3_label_header_s}</text>\n</g>\n")
        ...
        print('No label directory found, go back to general menu and create one or more labels')
