...
def add_templates_from_list(list_l, ask_questions):
    ...
    if ask_questions:
            ...
            if s == "b":
                ...
                try:
                    ...
                    if s_i in range(len(candidates_l)):
                        new_temp_abs_dir = os.path.join(
                            p1.p1_cntrct_abs_dir, candidates_l[s_i]
                        )
        ...
        print(
            "~~~ Select a template svg file in the graphic file browser -- mind the browser window could be hidden"
        )
...
def select_templates_context_func(prompt=True):
    ...
    print(
        "Editing contract #: ", p1.p1_d["cntrct_nr"] if p1.p1_d["cntrct_nr"] else None
    )
